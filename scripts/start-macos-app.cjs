...
const launcherExecutable = path.join(
    os.homedir(), 'Applications/Argus Launcher.app/Contents/MacOS/Electron');
...
// after that process started gets "No handler registered for 'argus:...'", and
...
// delivers argus:// deep links to the running copy, and a second live instance
// would fight over the same userData directory.
//
// Matched on the bundle's executable path, never on "Electron" or "argus".
// Argus Browser profiles and the per-profile launcher apps under
// ~/Applications/Argys Profiles are Electron/Chromium processes too, and
...
function stopRunningLauncher() {
  ...
  console.log(`Stopping ${pids.length} running Argus Launcher process(es) so this restart picks up electron/ changes.`);
