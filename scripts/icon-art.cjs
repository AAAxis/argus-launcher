...
function launcherSvg(themeKey) {
  const theme = THEMES[themeKey];
  // The helmet is 874x1124; 560 tall leaves it breathing room inside the tile.
  const height = 560;
  const scale = height / 1124;
  const x = CENTER - (874 * scale) / 2;
  const y = CENTER - height / 2;
  return wrap(`
  ${tileRect(theme.neutralTile)}
  <g transform="translate(${round(x)} ${round(y)}) scale(${round(scale, 4)})">
    ${helmetPaths(theme.neutralInk)}
  </g>
`);
}
...
// assets/argus-mark.svg is the brand file as exported, so its paths carry
...
function helmetPaths(fill) {
  const source = fs.readFileSync(
      path.join(__dirname, '..', 'assets', 'argus-mark.svg'), 'utf8');
  const paths = source.match(/<path\b[^>]*\/>/g);
  if (!paths || !paths.length) {
    throw new Error('assets/argus-mark.svg has no <path> elements to draw');
  }
  return paths
