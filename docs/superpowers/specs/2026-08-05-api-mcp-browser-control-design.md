...
Argus MCP server. Its top four recommendations have since been implemented —
...
- There is no click, type, press, scroll, hover, select or wait-for-selector
  tool. The only lever is `argus_eval` running `element.click()`, which fires
...
- `argus_list_tabs` returns tab ids **no other tool accepts**. Every page tool
...
- `folderId` is a settable argument on `argus_update_profile` with no tool that
...
page" fiction and makes `argus_list_tabs`'s output usable for the first time.
...
- `proxyMode` becomes a field on `update_profile`, so an agent can move a
  profile to Direct or Free Proxy. Today `argus_assign_proxy` always forces
...
for accounts the profile is logged into, and `argus_eval` already serves the
...
- **Advertisement.** `ARGUS_TOOL_PACKS=read,drive,manage` is written into the
...
`argus_delete_automation` stays in **manage** rather than moving to
...
- Each endpoint row shows **both faces of one capability** — `POST
  /v1/pages/click` and `argus_click` — with its pack badge. Click opens the
