# What an AI agent can do through the Monti MCP server
...
`~/.claude.json` points the `monti` MCP server at
`Monti Gate.app/Contents/Resources/app/electron/mcp/server.cjs`. On a
...
below. It now returns 25: `monti_profile_notes` and `monti_add_profile_note`
were added afterwards and are marked **[read]** in the table.

| Tool | Backed by | Verified? |
|---|---|---|
| `monti_list_profiles` | `GET /v1/profiles` | verified |
| `monti_get_profile` | `POST /v1/profiles/get` | verified |
| `monti_profile_session` | `POST /v1/profiles/cdp` | verified |
| `monti_launch_profile` | `POST /v1/profiles/launch-automation` | read only |
| `monti_close_profile` | `POST /v1/profiles/close-automation` | read only |
| `monti_update_profile` | `POST /v1/profiles/update` | read only |
| `monti_profile_notes` | `POST /v1/profiles/notes` | read only |
| `monti_add_profile_note` | `POST /v1/profiles/notes/add` | read only |
| `monti_list_proxies` | `GET /v1/proxies` | verified |
| `monti_assign_proxy` | `POST /v1/profiles/assign-proxy` | read only |
| `monti_check_proxy` | `POST /v1/proxies/check` | verified |
| `monti_list_tabs` | CDP `/json/list` | error path verified |
| `monti_navigate` | CDP `Page.navigate` | error path verified |
| `monti_read_page` | CDP `Runtime.evaluate` | read only |
| `monti_screenshot` | CDP `Page.captureScreenshot` | read only |
| `monti_eval` | CDP `Runtime.evaluate` | read only |
| `monti_list_automations` | `GET /v1/automations` | read only |
| `monti_automation_schema` | `GET /v1/automations/schema` | read only |
| `monti_get_automation` | `POST /v1/automations/get` | read only |
| `monti_create_automation` | `POST /v1/automations/create` | read only |
| `monti_update_automation` | `POST /v1/automations/update` | read only |
| `monti_delete_automation` | `POST /v1/automations/delete` | read only |
| `monti_run_automation` | `POST /v1/automations/run` | read only |
| `monti_table_columns` | `GET /v1/tables/columns` | read only |
| `monti_set_table_columns` | `POST /v1/tables/columns` | read only |
| `monti_list_connectors` | `GET /v1/connectors` | read only |
| `monti_create_connector` | `POST /v1/connectors/create` | read only |
| `monti_update_connector` | `POST /v1/connectors/update` | read only |
| `monti_delete_connector` | `POST /v1/connectors/delete` | read only |
| `monti_test_connector` | `POST /v1/connectors/test` | read only |
| `monti_list_folders` | `GET /v1/folders` | read only |
| `monti_list_statuses` | `GET /v1/statuses` | read only |
| `monti_telegram_status` | `GET /v1/telegram` | read only |
| `monti_set_telegram_pref` | `POST /v1/telegram/pref` | read only |
| `monti_set_telegram_bot` | `POST /v1/telegram/bot` | read only |
...
### Enumerate profiles
`monti_list_profiles` returned five real profiles. The payload is deliberately
...
`monti_get_profile` returns the whole row, including the complete fingerprint:
...
`monti_list_proxies` returned 11 proxies. Each row carries
...
`MontiProxy` row spread wholesale (`src/hooks/useAutomationBridge.ts:229-237`).
...
`monti_profile_session` answered entirely in the main process, no renderer round
trip (`electron/main.cjs:3736-3753`):
`{"status":true,"profileId":"…","running":false,"cdpUrl":null,"pid":null}`.

### Test a proxy
`monti_check_proxy` returned `{"ok":true,"ip":"206.251.200.232","country":"US","countryCode":"US","pingMs":497}`.
...
- CDP tool on a closed profile → `"Profile <id> is not open. Call monti_launch_profile first."`
...
- **`monti_launch_profile`** — `POST /v1/profiles/launch-automation`
...
- **`monti_close_profile`** — kills only sessions this process tracks; a window
  a human opened by hand is never reachable (`electron/main.cjs:3116-3120`,
  `3237-3261`).
- **`monti_update_profile`** — sets `name`, `tags` (capped at 5 by
  ...
  `monti_get_profile` and merge rather than sending one automation on its own.
...
- **`monti_automation_schema`** returns a `parameters` block alongside the step
...
- **`monti_create_automation` / `monti_update_automation`** take `parameters`,
...
- **`monti_list_automations`** returns each automation's `parameters`, so an
  agent can see what to pass without pulling the whole step tree.
- **`monti_update_profile.automationVars`** stores a profile's answers.
- **`monti_run_automation.vars`** overrides them for one run. A declared name is
...
**One call is one profile.** `monti_run_automation` takes a single `profileId`
...
cannot be read back out through `monti_automation_runs`. It is still stored as
...
- `monti_list_connectors` returns `{id, name, kind, category, is_default,
...
- `monti_create_connector` and `monti_update_connector` accept `config`. Anything
...
(`monti_telegram_status`, `monti_set_telegram_pref`) is the workspace's
...
Note `monti_delete_profile` is soft-delete only: the route accepts
...
- **Create a profile.** ~~No create route exists.~~ **Now possible** via
  `monti_create_profile` / `POST /v1/profiles/create`, which builds the row
...
- **See folders.** ~~No route lists folders.~~ **Now possible** via
  `monti_list_folders`, which returns the profile, proxy, cookie and automation
...
- **Restore an automation from Trash, or delete one permanently.**
  `monti_delete_automation` moves an automation to Trash, where it stops
...
- **See statuses.** ~~`custom_statuses` is never listed.~~ **Now possible** via
  `monti_list_statuses`, which returns the built-in labels per table plus the
  workspace's custom ones. `monti_update_profile.status` is still a free string,
...
- **Touch cookies at all.** The whole cookie library — upload a set, edit
  ...
  surface. `cookie_id` is readable via `monti_get_profile` and nothing more.
...
- **Edit fingerprints.** ~~The route exists but is deliberately withheld.~~
  **Now possible** via `monti_update_fingerprint`, which merges a whitelisted set
  of fingerprint fields into the stored one.
- **Set `proxy_mode`.** ~~`monti_update_profile` does not accept it.~~ **Now
  possible**: `monti_update_profile` accepts `proxyMode` (assigned/direct/
  free_proxy). Assigning a specific proxy is still `monti_assign_proxy`; switching
  to direct/free_proxy clears the proxy.
- **Set the start URL** — **now possible** via `monti_update_profile.startUrl`
  and `monti_create_profile.startUrl`. The profile's **login URL** is writable
  too (`monti_update_profile.loginUrl`). **Command-line switches and the
...
- **Link a member's Telegram.** `monti_telegram_status` reports whether the
  signed-in user is linked and `monti_set_telegram_pref` changes what they are
...
- **Control tabs.** `monti_list_tabs` returns tab ids, but no tool accepts one.
...
- **Interact with a page.** There is no click, type, hover, scroll, select,
  upload, wait-for-selector or cookie-jar tool. The only lever is `monti_eval`.

### 4c. Looks like an oversight, not intent

1. ~~**`monti_update_profile` declares a `notes` parameter that does nothing.**~~
   **Fixed.** The phantom `notes` parameter is gone; the tool's schema now
   declares only fields the route actually maps.
2. ~~**`monti_update_profile` forwards every argument it is given.**~~ **Fixed.**
...
3. **`monti_check_proxy` declares `type` and the route drops it.**
   ...
   reported dead.
4. **`monti_assign_proxy`'s description is wrong.** It says "direct connection
...
- That is a scope escape: move an out-of-scope profile into an in-scope folder
  with `monti_update_profile`, then `monti_get_profile` and
  `monti_launch_profile` it legitimately.

Scoping is also not a secret-hiding boundary: `monti_list_proxies` hands every
...
|---|---|
| Launcher not running | `"Monti Gate is not running, or its local API is not ready yet"` — `ECONNREFUSED` is translated (`electron/mcp/api.cjs:62-66`) |
| Launcher window closed (`mainWindow === null`, `main.cjs:923-928`) | HTTP 503 `"Monti Gate window is not open"` on every renderer-backed route (`main.cjs:3656,3674,3755`). On macOS the app survives window close, so the API stays up and returns 503 for reads/writes. |
| Window busy / renderer wedged | HTTP 504 `"Timed out waiting for Monti Gate to respond"` after **`AUTOMATION_REQUEST_TIMEOUT_MS = 20000`** (`main.cjs:2976`), surfaced verbatim as an `isError` tool result. The MCP HTTP client's own cap is 30 s (`electron/mcp/api.cjs:11`), so the server-side 504 normally wins. |
...
```
10:01  monti_list_profiles → 5 profiles;  monti_list_proxies → 11 proxies
10:06  monti_list_profiles → {"status": true, "profiles": []}
       monti_list_proxies  → {"status": true, "proxies": []}
       monti_get_profile("d77190bb-…")  → isError "Profile not found"
```
...
5. **The proxy to actually answer.** `spawnProfileUnchecked` runs a live
   ...
   `"Proxy <host>:<port> did not respond … Fix the proxy in Monti Gate and
...
agent's correct recovery is `monti_profile_session`, not a retry.
...
- Command timeout 30 s, connect timeout 10 s, navigation settle 30 s
  (`cdp.cjs:22-26`).
- `monti_read_page` truncates at 20 000 characters and returns `innerText`, so
  it sees no `<title>`-less SPA shells, no shadow DOM, no iframes, and nothing
  the layout hides.
- `monti_screenshot` defaults to JPEG q70 explicitly to keep bytes out of the
...
1. **Proxy credentials leave the machine with the agent.** One
   `monti_list_proxies` call puts every proxy username and password into the
   agent's context, and therefore into whatever the agent's transcript is
   uploaded to. **[verified]**
2. **`monti_eval` is unrestricted JavaScript in a session that is often logged
...
6. **`monti_check_proxy` is an outbound-connection primitive.** It takes an
...
2. **Stop returning proxy passwords from `GET /v1/proxies`.** Redact
   ...
   does with the current tool set needs them — `monti_assign_proxy` takes a
...
5. **Add page-interaction tools: click, type, wait-for-selector.** Right now
   every interaction has to go through `monti_eval` and `element.click()`, which
   ...
   the product.
6. **Make tabs addressable.** `monti_list_tabs` returns ids no other tool
...
8. **Drop the dead `notes` parameter, honour `type` in `proxies/check`, and set
   `additionalProperties: false` on `monti_update_profile`.** Three small
