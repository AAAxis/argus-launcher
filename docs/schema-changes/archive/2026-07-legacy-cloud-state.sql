alter table public.monti_cloud_state
...
create table if not exists public.monti_monitoring_results (
...
create index if not exists monti_monitoring_results_user_run_idx
on public.monti_monitoring_results (user_id, run_id);

alter table public.monti_monitoring_results enable row level security;

drop policy if exists "users can read their own monitoring results" on public.monti_monitoring_results;
create policy "users can read their own monitoring results"
on public.monti_monitoring_results
for select
to authenticated
using (auth.uid() = user_id);

drop policy if exists "users can insert their own monitoring results" on public.monti_monitoring_results;
...
drop policy if exists "users can delete their own monitoring results" on public.monti_monitoring_results;
