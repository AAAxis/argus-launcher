...
export function useAutomationActions(
  ...
  const {runs, startRun, cancelRun, waitForRun} = useAutomationRuns(orgId, signedIn,
      ...
      // forward).
      //
      // Notify-on-finish delivered to Monti, only when the event carries a
  ...
  async function linkTelegram() {
    ...
    const welcome =
      'Welcome! Your Telegram is now linked to Monti.\n\n' +
    ...
    toast.notify('Telegram linked. Monti can message you now.', {tone: 'ok'});
  }
  ...
  async function run(
    ...
    const result = await startRun(automation, profile, {
      ...
      buildLaunch: async (cdpPort) => {
        ...
        // monti-launch.json nor monti-session.json, and the Monti Helper in the
        // window this run opens answers every question with "This window was not
        // launched from Monti Launcher" -- no proxy card, no automations, and,
