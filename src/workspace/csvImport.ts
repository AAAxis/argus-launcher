...
export type ImportRow = {
  ...
  proxy: Omit<ArgusProxy, 'id' | 'name'> | null;
  ...
  fingerprint: NonNullable<ArgusProfile['fingerprint']>;
...
export type ImportPlan = {
  ...
  proxies: ArgusProxy[];
  folders: ArgusFolder[];
  upsertProxies: ArgusProxy[];
  newFolders: ArgusFolder[];
  touchedProfiles: Array<{profile: ArgusProfile; exists: boolean}>;
  result: ImportResult;
};
...
export function profileExportRow(
    ...
    folder: ArgusFolder | null): Record<string, string> {
  return {
    name: profile.name || '',
...
export function planCsvImport(
  ...
  // alone.
  const upsertProxies: ArgusProxy[] = [];
