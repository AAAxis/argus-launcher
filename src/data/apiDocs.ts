...
// Monti Gate Browser API example.
// Keep Monti Gate open and signed in while running this script.
...
async function main() {
  ...
  if (!profile) {
    throw new Error('Create a profile in Monti Gate first.');
  }
...
export function agentPrompt(tool: AgentTool) {
  ...
  return [
    ...
    '',
    'Monti manages anti-detect browser profiles. Each profile is an isolated',
