...
describe('entryMatches', () => {
  ...
  it('matches on a tool name, including one with no route', () => {
    expect(entryMatches(find('argus_screenshot'), 'argus_screen')).toBe(true);
    expect(entryMatches(find('GET /v1/profiles'), 'argus_list_profiles')).toBe(true);
  });

  it('matches on the label, which is what someone types when they know the verb', () => {
    expect(entryMatches(find('argus_eval'), 'javascript')).toBe(true);
  });

  it('is case-insensitive, and trims, so a pasted needle still matches', () => {
    expect(entryMatches(find('argus_navigate'), 'URL')).toBe(true);
    expect(entryMatches(find('argus_screenshot'), '  ARGUS_Screenshot ')).toBe(true);
