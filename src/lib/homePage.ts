...
// The Monti mark that sits above the search box, imported ?raw rather than as a
...
<button type="button" class="auto-open icon-button" title="Open ${name} in Monti Gate" aria-label="Open ${name} in Monti Gate">${EXTERNAL_ICON}</button>
</div>
</article>`;
      })
      .join('') : '';
...
/* The Monti mark, over the profile name and the search box. The launcher's own
...
/* ── Section labels ─────────────────────────────────────────────────────────
   ...
   It used to be the Monti helmet, at 18px, which was the header mark from 30px
...
<h1>${safeName}</h1>
<p class="sub">Anonymous Monti Browser session</p>
      ...
      post('/v1/proxies/recheck-from-page', {runToken: RUN.token})
        .then(function (result) {
          if (!result.ok || !result.body.status) {
            /* The launcher's own words, not a sentence invented here. Every
               refusal on this route already carries one -- "Monti Launcher is
        ...
        .catch(function (error) {
          ...
          line.textContent = error && error.fromLauncher ? error.message :
            'The check did not complete. Try again from the Monti panel.';
        })
        .then(function () {
