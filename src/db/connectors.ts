...
export async function list(orgId: string): Promise<ArgusConnector[]> {
...
export async function create(orgId: string, connector: ArgusConnector): Promise<void> {
  const client = requireClient();
  const {error} = await client.from('connectors').insert(connectorToRow(orgId, connector));
  raise(error, 'connectors.create');
}

export async function replace(orgId: string, connector: ArgusConnector): Promise<void> {
  const client = requireClient();
...
export async function save(
    orgId: string, connector: ArgusConnector, exists: boolean): Promise<void> {
