...
## The two copies of the Monti mark

| File | Route | Used by |
|---|---|---|
| `monti-mark.png` | URL, as a CSS `mask-image` | `--monti-mark` in `styles.css`, which `.brand-mark` (sidebar), `.extension-mark.is-monti` and `.start-brand-mark` all mask with |
| `monti-mark.svg` | `?raw`, inlined into a document | `lib/homePage.ts` only |
...
from `../../assets/monti-mark.svg` (= `../../../brand/monti.svg`, md5
...
2. The clipPath id is `monti-mark-clip`, not Figma's `clip0_4261_2`. It is the
...
Then open a generated `home.html` (`<user data dir>/MontiHome/home.html`) in
