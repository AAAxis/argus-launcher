// What the launcher knows about which Argus Browser build is installed, and
...
// `.argus-browser-build` below, which recorded only the hash -- enough to
// answer "is this current?" and nothing else, which is why the UI could never
// show a version or an install date.
const INSTALL_RECORD_FILE = '.argus-browser-install.json';
const LEGACY_BUILD_ID_FILE = '.argus-browser-build';
...
// bundled one, or a hand-installed /Applications/Argus.app -- and in that case
